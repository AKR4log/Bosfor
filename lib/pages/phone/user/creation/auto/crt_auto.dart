...
class _CRTAutoState extends State<CRTAuto> {
  ...
  Future<Null> _uploadImages() async {
    _imageList.forEach((f) async {
      final firebase_storage.Reference _ref = firebase_storage
          .FirebaseStorage.instance
          .ref()
          .child('360')
          .child(path.basename(f.path));
      final firebase_storage.UploadTask uploadTask = _ref.putFile(File(f.path));
      uploadTask.snapshotEvents.listen((event) {}).onData((snapshot) {
        if (snapshot.state == firebase_storage.TaskState.success) {
          // _ref.getDownloadURL().then((value) {
          //   setState(() {
          //     _imageListURL.add(value);
          //   });
          // });
          _ref.getData().then((value) {
            setState(() {
              _imageListURL.add(value);
            });
          });
                ...
                return Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Center(
                        child: Text(
                          'Выберите до 52 фото для создания 360 фото',
          ...
          children: [
            ...
            Container(
              ...
              child: TextButton(
                ...
                child: Text('Добавить фото 360'),
                onPressed: () => getImageFromGallery(),
              ),
            ),
            _imageList != null && _imageList.length >= 1 && !isUploaded
                ? Container(
                    width: double.infinity,
                    height: 45,
                    margin: EdgeInsets.symmetric(vertical: 5),
            ...
            finished_first_photo != null ||
                    isUploaded && _imageListURL != null && _imageListURL != ''
                ? Container(
